...
const DataReportDownload = ({ parameter, user, module, data, logo }) => {

  const handleSendEmail = async () => {
    try {
      // 1️⃣ Generate the PDF blob
      const doc = <DataReport parameter={parameter} user={user} module={module} data={data}/>;
  ...
  const handleDownload = async () => {
    const doc = <DataReport parameter={parameter} user={user} module={module} data={data} logo={logo}/>;
    const blob = await pdf(doc).toBlob();
...
export default DataReportDownload;
